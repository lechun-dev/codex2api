...
func (db *DB) ensureAccountDailyUsageTable(ctx context.Context) error {
	...
	breakdownDefinition := "TEXT NOT NULL DEFAULT '[]'"
	surfacesDefinition := "TEXT NOT NULL DEFAULT '{}'"
	if db.isSQLite() {
		timeType = "TIMESTAMP"
		boolType = "INTEGER"
		boolDefault = "0"
		...
		breakdownDefinition = "MEDIUMTEXT NULL"
		surfacesDefinition = "MEDIUMTEXT NULL"
	}
	...
	)`, boolType, boolDefault, jsonDefinition, jsonDefinition, timeType, breakdownDefinition, surfacesDefinition)
	if _, err := db.conn.ExecContext(ctx, ddl); err != nil {
		return err
	}
	if db.isMySQL() {
		if err := db.ensureMySQLIndex(ctx, "account_daily_usage", "idx_account_daily_usage_day", "CREATE INDEX idx_account_daily_usage_day ON account_daily_usage(day)"); err != nil {
			return err
		}
	} else if _, err := db.conn.ExecContext(ctx, `CREATE INDEX IF NOT EXISTS idx_account_daily_usage_day ON account_daily_usage(day)`); err != nil {
		return err
	}
	// 2026-09 新增的拆分三列：老库靠幂等加列补齐。
	breakdownColumns := []struct{ name, def string }{
		{"breakdown_percent", "DOUBLE PRECISION NOT NULL DEFAULT 0"},
		{"breakdown_json", breakdownDefinition},
		{"surfaces_json", surfacesDefinition},
	}
	for _, column := range breakdownColumns {
		if db.isMySQL() {
			if err := db.ensureMySQLColumn(ctx, "account_daily_usage", column.name, column.def); err != nil {
				return err
			}
			continue
		}
...
func (db *DB) UpsertAccountDailyBreakdown(ctx context.Context, input AccountDailyBreakdownInput) error {
	...
	_, err = db.conn.ExecContext(ctx, `INSERT INTO account_daily_usage (
		account_id, day, synced_at, breakdown_percent, breakdown_json, surfaces_json, clients_json, models_json
	) VALUES ($1,$2,$3,$4,$5,$6,'[]','[]')
	ON CONFLICT (account_id, day) DO UPDATE SET
...
const accountDailyUsageSelect = `SELECT account_id, day, credits, users, threads, turns,
	uncached_input_tokens, cached_input_tokens, output_tokens, total_tokens,
	settled, clients_json, models_json, synced_at,
	breakdown_percent, COALESCE(breakdown_json, '[]'), COALESCE(surfaces_json, '{}') FROM account_daily_usage`
