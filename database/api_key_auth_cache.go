...
func (db *DB) ensureAPIKeyAuthCacheSchema(ctx context.Context) error {
	if db.isMySQL() {
		return db.ensureMySQLAPIKeyAuthCacheSchema(ctx)
	}
...
func (db *DB) ensureMySQLAPIKeyAuthCacheSchema(ctx context.Context) error {
	// 2026-09-11 coder(lq): MySQL 5.6 permits only one trigger for each table/event/timing pair, so the scheduler triggers maintain this state.
	if _, err := db.conn.ExecContext(ctx, apiKeyAuthCacheStateMySQLDDL()); err != nil {
		return err
	}
	if _, err := db.conn.ExecContext(ctx, `INSERT IGNORE INTO api_key_auth_cache_state(id,namespace,generation,key_count)
		SELECT 1,?,1,COUNT(*) FROM api_keys`, uuid.NewString()); err != nil {
		return err
	}
	// 2026-09-11 coder(lq): Reconcile pre-existing state after upgrades so stale key counts cannot keep authentication in the wrong mode.
	_, err := db.conn.ExecContext(ctx, `UPDATE api_key_auth_cache_state
		SET generation=generation+1,key_count=(SELECT COUNT(*) FROM api_keys)
		WHERE id=1`)
	return err
}

func apiKeyAuthCacheStateMySQLDDL() string {
	return `CREATE TABLE IF NOT EXISTS api_key_auth_cache_state (
