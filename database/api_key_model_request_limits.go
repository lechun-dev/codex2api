...
func (db *DB) ConsumeAPIKeyModelRequest(ctx context.Context, keyID int64, requestID, effectiveModel string, rules []APIKeyModelRequestLimit, now time.Time) (*APIKeyModelRequestExhaustion, error) {
	...
	err := db.withWriteTx(ctx, func(tx *sql.Tx) error {
		...
		for _, item := range charges {
			if _, err := tx.ExecContext(ctx, `INSERT INTO api_key_model_request_counters (api_key_id,rule_id,window_start,reset_at,used_requests) VALUES ($1,$2,$3,$4,1) ON CONFLICT (api_key_id,rule_id,window_start) DO UPDATE SET used_requests=used_requests+1`, keyID, item.rule.ID, item.start.Unix(), item.end.Unix()); err != nil {
				return err
			}
