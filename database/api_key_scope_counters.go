...
func (db *DB) ResetAPIKeyScopeCounter(ctx context.Context, apiKeyID int64, scopeType string, scopeID int64) error {
	...
	query := db.resetAPIKeyScopeCounterSQL()
	_, err := db.conn.ExecContext(ctx, query, apiKeyID, scopeType, scopeID)
	return err
}

func (db *DB) resetAPIKeyScopeCounterSQL() string {
	if db.isMySQL() {
		return `
...
func (db *DB) apiKeyScopeCounterUpsertSQL() (string, string) {
	if db.isMySQL() {
		...
		groupQuery := `
		INSERT INTO api_key_scope_counters (api_key_id, scope_type, scope_id, used_cost, used_tokens, used_requests, updated_at)
		SELECT $1, '` + APIKeyScopeTypeGroup + `', m.group_id, $3, $4, $5, NOW()
		...
		ON DUPLICATE KEY UPDATE
			...
			used_requests = COALESCE(used_requests, 0) + VALUES(used_requests),
			updated_at = NOW()`
		return accountQuery, groupQuery
	}
