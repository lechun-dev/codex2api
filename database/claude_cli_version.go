...
func (db *DB) UpdateClaudeSyncedCLIVersion(ctx context.Context, version string) error {
	...
	return db.withSQLiteWriteLock(ctx, func() error {
		...
		switch {
		case db.isMySQL():
			// 2026-09-03 coder(lq): MySQL 5.6 uses its native upsert syntax; PostgreSQL EXCLUDED is invalid there.
			query = `
				INSERT INTO system_settings (id, claude_synced_cli_version) VALUES (1, ?)
