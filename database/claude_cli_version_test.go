...
func TestUpdateClaudeSyncedCLIVersionUsesMySQL56Upsert(t *testing.T) {
	capture := &mysqlCaptureDriver{}
	db := newMySQLCaptureDB(t, capture)

	if err := db.UpdateClaudeSyncedCLIVersion(context.Background(), "2.1.300"); err != nil {
		t.Fatalf("UpdateClaudeSyncedCLIVersion() error = %v", err)
	}
	if !strings.Contains(capture.query, "ON DUPLICATE KEY UPDATE") {
		t.Fatalf("Claude CLI version upsert is not MySQL-compatible: %s", capture.query)
	}
