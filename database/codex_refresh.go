...
func (db *DB) codexRefreshRows(ctx context.Context, tx *sql.Tx, rt string) ([]AccountRow, error) {
	...
	if db.isMySQL() {
		// MySQL 5.6 has no JSON operators; scan credentials in Go like GetAllRefreshTokens.
		query := `SELECT id, credentials, credential_generation FROM accounts WHERE status <> 'deleted' ORDER BY id FOR UPDATE`
		...
		for rows.Next() {
			...
			upstream := strings.ToLower(strings.TrimSpace(credentialStringFromMap(row.Credentials, "upstream_type")))
			if upstream != "" && upstream != "codex" {
				continue
			}
			if credentialStringFromMap(row.Credentials, "refresh_token") != rt {
				continue
			}
...
func (db *DB) BeginCodexRefresh(ctx context.Context, accountID, generation int64, rt string) (*CodexRefreshAttempt, error) {
	...
	err := db.withSQLiteWriteLock(ctx, func() error {
		...
		var res sql.Result
		if db.isMySQL() {
			res, err = tx.ExecContext(ctx, `INSERT IGNORE INTO codex_oauth_refresh_attempts(rt_fingerprint, attempt_id, started_at)
				VALUES ($1,$2,$3)`, attempt.Fingerprint, attempt.ID, time.Now().Unix())
		} else {
...
func (db *DB) writeCodexRefreshCredentials(ctx context.Context, tx *sql.Tx, row AccountRow, credentials map[string]any, advance bool) error {
	encoded, err := marshalCredentialsForStorage(credentials)
	if err != nil {
		return err
	}
