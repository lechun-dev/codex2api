...
func continuousRetryPolicySelectQuery(forUpdate bool) string {
	query := `
		SELECT COALESCE(continuous_retry_policy, '{"enabled":false,"catch_all":false,"categories":["transport","http_429","http_5xx","stream_error"],"status_codes":[],"error_codes":[],"max_duration_seconds":600}')
...
// UpdateContinuousRetryPolicy applies partial admin edits under a row lock so
// concurrent saves cannot drop fields added by another request.
func (db *DB) UpdateContinuousRetryPolicy(ctx context.Context, update ContinuousRetryPolicyUpdate) (ContinuousRetryPolicy, error) {
	...
	err := db.withSQLiteWriteLock(ctx, func() error {
		...
		var raw string
		if err := tx.QueryRowContext(ctx, continuousRetryPolicySelectQuery(!db.isSQLite())).Scan(&raw); err != nil {
			if !errors.Is(err, sql.ErrNoRows) {
				return err
			}
		} else {
			current = ParseContinuousRetryPolicy(raw)
		}
		...
		if update.Categories != nil {
			next.Categories = *update.Categories
		}
		if update.StatusCodes != nil {
			next.StatusCodes = *update.StatusCodes
		}
		if update.ErrorCodes != nil {
			next.ErrorCodes = *update.ErrorCodes
		}
		...
		if _, err := tx.ExecContext(ctx, `
			...
			WHERE id = 1
		`, EncodeContinuousRetryPolicy(next)); err != nil {
			return err
		}
