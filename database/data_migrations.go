...
func (db *DB) runDataMigrations(ctx context.Context) error {
	...
	if err := db.runDataMigrationOnce(ctx, dataMigrationOAuthIdentityDedupeV2, func(ctx context.Context, tx *sql.Tx) error {
	...
	for i := range accounts {
		...
		if email != "" && strings.TrimSpace(credentialStringFromMap(account.credentials, "workspace_id")) == "" {
			...
			if workspaceID != "" && strings.EqualFold(tokenEmail, email) {
				...
				query := db.workspaceIdentityCredentialsUpdateSQL()
...
func (db *DB) workspaceIdentityCredentialsUpdateSQL() string {
	if db != nil && !db.isSQLite() && !db.isMySQL() {
		return `UPDATE accounts SET credentials = $1::jsonb WHERE id = $2`
	}
	return `UPDATE accounts SET credentials = $1 WHERE id = $2`
}
