...
func (db *DB) SeedGrokMaintenanceJobs(ctx context.Context, dueAt time.Time) error {
	...
	case db.isSQLite():
		...
		WHERE status<>'deleted' AND COALESCE(error_message,'')<>'deleted'
		  AND COALESCE(enabled,true) AND LOWER(COALESCE(json_extract(credentials,'$.upstream_type'),''))='grok'
		ON CONFLICT(entity_id,job_kind) DO NOTHING`
	case db.isMySQL():
		// 2026-09-03 coder(lq): MySQL 5.6 has no JSON operators here, so match
		// the stored credentials blob with a conservative text predicate.
		query = `
		INSERT IGNORE INTO maintenance_jobs(entity_id,job_kind,due_at,updated_at)
		SELECT id,$1,$2,CURRENT_TIMESTAMP FROM accounts
		WHERE status<>'deleted' AND COALESCE(error_message,'')<>'deleted'
		  AND COALESCE(enabled,1)<>0
		  AND LOWER(COALESCE(CAST(credentials AS CHAR), '')) REGEXP '"upstream_type"[[:space:]]*:[[:space:]]*"grok"'`
	default:
		query = `
		INSERT INTO maintenance_jobs(entity_id,job_kind,due_at,updated_at)
		SELECT id,$1,$2,CURRENT_TIMESTAMP FROM accounts
		WHERE status<>'deleted' AND COALESCE(error_message,'')<>'deleted'
		  AND COALESCE(enabled,true) AND LOWER(COALESCE(credentials->>'upstream_type',''))='grok'
		ON CONFLICT(entity_id,job_kind) DO NOTHING`
	}
...
// ClaimMaintenanceJobs leases due rows. PostgreSQL uses SKIP LOCKED; MySQL 5.6
// falls back to a single select-for-update transaction; SQLite serializes the
// short select/update transaction through its existing write gate.
	...
	if db.isMySQL() {
		...
		err := db.withWriteTx(ctx, func(tx *sql.Tx) error {
			rows, err := tx.QueryContext(ctx, `
				SELECT entity_id,job_kind,due_at,lease_owner,lease_until,attempts,last_error
				FROM maintenance_jobs
				WHERE job_kind=$1 AND due_at<=$2 AND (lease_until IS NULL OR lease_until<=$2)
				ORDER BY due_at,entity_id
				LIMIT $3 FOR UPDATE`,
				jobKind, db.timeArg(now), limit)
			if err != nil {
				return err
			}
