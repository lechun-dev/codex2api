...
// 2026-09-06 coder(lq): Keep the MySQL schema independent of PostgreSQL foreign-key and text definitions used by the generic schema.
func modelCapabilitiesMySQLDDL() string {
	return `CREATE TABLE IF NOT EXISTS model_capability_snapshots (
...
func (db *DB) SaveModelCapabilities(ctx context.Context, snapshot ModelCapabilitySnapshot) error {
	...
	return db.withWriteTx(ctx, func(tx *sql.Tx) error {
		...
		upsertQuery := `INSERT INTO model_capability_snapshots(account_id,credential_generation,observed_at,models_json)
	 VALUES($1,$2,$3,$4) ON CONFLICT(account_id) DO UPDATE SET credential_generation=excluded.credential_generation,observed_at=excluded.observed_at,models_json=excluded.models_json`
		if db.isMySQL() {
			// 2026-09-06 coder(lq): MySQL 5.6 has no PostgreSQL ON CONFLICT/EXCLUDED syntax; use its native upsert explicitly for capability snapshots.
			upsertQuery = `INSERT INTO model_capability_snapshots(account_id,credential_generation,observed_at,models_json)
	 VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE credential_generation=VALUES(credential_generation),observed_at=VALUES(observed_at),models_json=VALUES(models_json)`
		}
		_, err = tx.ExecContext(ctx, upsertQuery, snapshot.AccountID, generation, snapshot.ObservedAt, string(encoded))
		return err
	})
}
