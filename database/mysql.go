...
func (db *DB) migrateMySQL(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS usage_logs (
			...
			user_billing_mode VARCHAR(32) DEFAULT '',
			image_unit_price DOUBLE DEFAULT 0,
			billed_image_count INT DEFAULT 0,
	...
	columns := []mysqlColumnDefinition{
		...
		{"usage_logs", "cache_write_1h_tokens", "INT DEFAULT 0"},
		...
		{"usage_logs", "user_billing_mode", "VARCHAR(32) DEFAULT ''"},
		{"usage_logs", "image_unit_price", "DOUBLE DEFAULT 0"},
		{"usage_logs", "billed_image_count", "INT DEFAULT 0"},
		...
		{"api_keys", "enabled", "TINYINT(1) NOT NULL DEFAULT 1"},
		...
		{"system_settings", "first_token_mode", "VARCHAR(20) DEFAULT 'loose'"},
...
func systemSettingsMySQLDDL() string {
	return `CREATE TABLE IF NOT EXISTS system_settings (
		...
		first_token_mode VARCHAR(20) DEFAULT 'loose',
