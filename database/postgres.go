...
func (db *DB) CleanErrorProxies(ctx context.Context) (ProxyErrorCleanupResult, error) {
	...
	err := db.withSQLiteWriteLock(ctx, func() error {
		...
		proxyURLArgs := make([]interface{}, len(proxyURLs))
		for i, proxyURL := range proxyURLs {
			proxyURLArgs[i] = proxyURL
		}
		...
		rows, err = tx.QueryContext(ctx, accountQuery, proxyURLArgs...)
		if err != nil {
			return err
		}
		...
		if len(accountIDs) > 0 {
			unbindArgs := append(argsFromInt64s(accountIDs), proxyURLArgs...)
			unbindQuery := fmt.Sprintf(`
				UPDATE accounts
				SET proxy_url = '', updated_at = CURRENT_TIMESTAMP
				WHERE id IN (%s)
				  AND TRIM(COALESCE(proxy_url, '')) IN (%s)`,
				strings.Join(dbPlaceholders(db.isSQLite(), 1, len(accountIDs)), ","),
				strings.Join(dbPlaceholders(db.isSQLite(), len(accountIDs)+1, len(proxyURLs)), ","),
			)
			if _, err := tx.ExecContext(ctx, unbindQuery, unbindArgs...); err != nil {
				return err
			}
			...
			updatedQuery := fmt.Sprintf(`
				...
				FROM accounts
				WHERE id IN (%s)
				  AND TRIM(COALESCE(proxy_url, '')) = ''
				ORDER BY id`,
				strings.Join(dbPlaceholders(db.isSQLite(), 1, len(accountIDs)), ","),
			)
			rows, err = tx.QueryContext(ctx, updatedQuery, argsFromInt64s(accountIDs)...)
			if err != nil {
				return err
			}
			for rows.Next() {
		...
		remainingQuery := fmt.Sprintf(
			`SELECT id FROM proxies WHERE id IN (%s)`,
			strings.Join(dbPlaceholders(db.isSQLite(), 1, len(proxyIDs)), ","),
		)
		rows, err = tx.QueryContext(ctx, remainingQuery, argsFromInt64s(proxyIDs)...)
		if err != nil {
			return err
		}
		remainingProxyIDs := make(map[int64]struct{})
		for rows.Next() {
			var proxyID int64
			...
			remainingProxyIDs[proxyID] = struct{}{}
		}
		if err := rows.Close(); err != nil {
			return err
		}
		...
		for i, proxyID := range proxyIDs {
			if _, exists := remainingProxyIDs[proxyID]; exists {
				continue
			}
			result.Deleted++
			result.DeletedProxyURLs = append(result.DeletedProxyURLs, proxyURLs[i])
		}
...
func (db *DB) InsertUsageLog(ctx context.Context, input *UsageLogInput) error {
	...
	storeUsageLog := db.shouldStoreUsageLog(input)

	billingServiceTier := usageLogBillingServiceTier(input)
	...
	accountBilled := UsageLogBilledCost(input)

	// 用户计费金额与账号计费金额相同（简化版，未来可支持倍率）
	userBilled := accountBilled
	if !storeUsageLog && (input.APIKeyID <= 0 || userBilled <= 0 || input.StatusCode == 499) {
		return nil
	}
	...
	entry := usageLogEntry{
		...
		Channel:                clampUsageLogText(input.Channel, usageLogChannelMaxLen),
		ClientIP:               clampUsageLogText(input.ClientIP, usageLogShortTextMaxLen),
		SessionID:              clampUsageLogText(input.SessionID, usageLogIDMaxLen),
		ConversationID:         clampUsageLogText(input.ConversationID, usageLogIDMaxLen),
		PreviousResponseID:     clampUsageLogText(input.PreviousResponseID, usageLogIDMaxLen),
		RequestText:            input.RequestText,
		ClientUserAgent:        input.ClientUserAgent,
		UpstreamUserAgent:      input.UpstreamUserAgent,
		UserAgentOverridden:    input.UserAgentOverridden,
		InternalReason:         clampUsageLogText(input.InternalReason, usageLogShortTextMaxLen),
		ParentRequestID:        clampUsageLogText(input.ParentRequestID, usageLogRequestIDMaxLen),
		Endpoint:               clampUsageLogText(input.Endpoint, usageLogTextMaxLen),
		Model:                  clampUsageLogText(input.Model, usageLogTextMaxLen),
		EffectiveModel:         clampUsageLogText(input.EffectiveModel, usageLogTextMaxLen),
		...
		WsAcquireMs:            input.WsAcquireMs,
		ReasoningEffort:        clampUsageLogText(input.ReasoningEffort, usageLogTextMaxLen),
		InboundEndpoint:        clampUsageLogText(input.InboundEndpoint, usageLogTextMaxLen),
		UpstreamEndpoint:       clampUsageLogText(input.UpstreamEndpoint, usageLogTextMaxLen),
		Stream:                 input.Stream,
		Compact:                input.Compact,
		HasCompactionHistory:   input.HasCompactionHistory,
		...
		APIKeyName:             clampUsageLogText(input.APIKeyName, usageLogAPIKeyNameMaxLen),
		APIKeyMasked:           clampUsageLogText(input.APIKeyMasked, usageLogShortTextMaxLen),
		ImageCount:             input.ImageCount,
		ImageWidth:             input.ImageWidth,
		ImageHeight:            input.ImageHeight,
		ImageBytes:             input.ImageBytes,
		ImageFormat:            clampUsageLogText(input.ImageFormat, usageLogTextMaxLen),
		ImageSize:              clampUsageLogText(input.ImageSize, usageLogImageSizeMaxLen),
		...
		PromptPolicyIncidentID: clampUsageLogText(input.PromptPolicyIncidentID, usageLogShortTextMaxLen),
	}
...
func (db *DB) sanitizeUsageLogEntryForStorage(entry *usageLogEntry) {
	...
	entry.Channel = sanitizeMySQLTextValue(entry.Channel)
	...
	entry.ClientUserAgent = sanitizeMySQLTextValue(entry.ClientUserAgent)
	entry.UpstreamUserAgent = sanitizeMySQLTextValue(entry.UpstreamUserAgent)
...
func (db *DB) insertSQLiteUsageLogBatch(ctx context.Context, batch []usageLogEntry) error {
	...
	if len(logsToStore) > 0 {
		stmt, err := tx.PrepareContext(ctx,
			`INSERT INTO usage_logs (account_id, channel, client_ip, session_id, conversation_id, previous_response_id, request_text, endpoint, model, effective_model, prompt_tokens, completion_tokens, total_tokens, status_code, duration_ms,
		...
		for _, e := range logsToStore {
			if _, err := stmt.ExecContext(ctx, e.AccountID, e.Channel, e.ClientIP, e.SessionID, e.ConversationID, e.PreviousResponseID, e.RequestText, e.Endpoint, e.Model, e.EffectiveModel, e.PromptTokens, e.CompletionTokens, e.TotalTokens, e.StatusCode, e.DurationMs,
...
// PostgreSQL 单条语句最多 65535 个 bind 参数；usage_logs 当前每行 53 个参数，
...
func (db *DB) batchInsertLogsChunk(ctx context.Context, execer sqlExecer, batch []usageLogEntry) error {
	...
	for _, e := range batch {
		...
		valueArgs = append(valueArgs, e.AccountID, e.Channel, e.ClientIP, e.SessionID, e.ConversationID, e.PreviousResponseID, e.RequestText, e.Endpoint, e.Model, e.EffectiveModel, e.PromptTokens, e.CompletionTokens, e.TotalTokens, e.StatusCode, e.DurationMs,
			e.InputTokens, e.OutputTokens, e.ReasoningTokens, e.FirstTokenMs, e.WsAcquireMs, e.ReasoningEffort, e.InboundEndpoint, e.UpstreamEndpoint, e.Stream, e.Compact, e.HasCompactionHistory, e.CachedTokens, e.ServiceTier,
			e.RequestedServiceTier, e.ActualServiceTier, e.BillingServiceTier,
			e.APIKeyID, e.APIKeyName, e.APIKeyMasked, e.ImageCount, e.ImageWidth, e.ImageHeight, e.ImageBytes, e.ImageFormat, e.ImageSize, e.AccountBilled, e.UserBilled,
			e.IsRetryAttempt, e.AttemptIndex, e.UpstreamErrorKind, e.ErrorMessage, e.ViaWebsocket,
	...
	query := fmt.Sprintf(`INSERT INTO usage_logs (account_id, channel, client_ip, session_id, conversation_id, previous_response_id, request_text, endpoint, model, effective_model, prompt_tokens, completion_tokens, total_tokens, status_code, duration_ms,
		input_tokens, output_tokens, reasoning_tokens, first_token_ms, ws_acquire_ms, reasoning_effort, inbound_endpoint, upstream_endpoint, stream, compact, has_compaction_history, cached_tokens, service_tier,
		requested_service_tier, actual_service_tier, billing_service_tier,
		api_key_id, api_key_name, api_key_masked, image_count, image_width, image_height, image_bytes, image_format, image_size, account_billed, user_billed,
		is_retry_attempt, attempt_index, upstream_error_kind, error_message, via_websocket,
...
func (db *DB) ClearUsageLogs(ctx context.Context) error {
	...
	defer tx.Rollback()
	if db.isMySQL() {
		// InnoDB locks the current maximum row and the supremum gap so inserts
		// cannot race the baseline snapshot and deletion.
		var lastID int64
		err := tx.QueryRowContext(ctx, `SELECT id FROM usage_logs ORDER BY id DESC LIMIT 1 FOR UPDATE`).Scan(&lastID)
		if err != nil && !errors.Is(err, sql.ErrNoRows) {
			return err
		}
	} else if !db.isSQLite() {
	...
	rollupQuery := `SELECT total_requests, total_tokens, prompt_tokens, completion_tokens,
		cached_tokens, cache_hit_requests, first_token_ms_sum, first_token_samples, account_billed, user_billed
		FROM usage_stats_rollup WHERE channel=''`
	if db.isMySQL() {
		rollupQuery += ` FOR UPDATE`
	}
	if err := tx.QueryRowContext(ctx, rollupQuery).Scan(&rollup.TotalRequests, &rollup.TotalTokens,
