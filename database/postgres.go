...
const (
	...
	// 2026-09-11 coder(lq): Keep local trace fields and upstream image billing fields in the shared insert shape.
	usageLogInsertColumnCount = 66
	// 2026-09-11 coder(lq): Derive the batch ceiling from the real column count so PostgreSQL never exceeds 65535 bind parameters.
...
func (db *DB) ensureUsageLogsGenerationIndex(parent context.Context) error {
	if db.isMySQL() {
		// MySQL creates this index synchronously during its normal schema pass.
		// The PostgreSQL-only CONCURRENTLY/pg_catalog probing below is invalid on
		// MySQL 5.6 and must never run on that driver.
		return nil
	}
...
func (db *DB) ensureUsageStatsRollup(ctx context.Context) error {
	...
	if db.isSQLite() {
		...
		if err := db.ensureMySQLColumn(ctx, "usage_stats_rollup_state", "aggregation_version", "INT NOT NULL DEFAULT 1"); err != nil {
			return err
		}
...
func (db *DB) InsertUsageLog(ctx context.Context, input *UsageLogInput) error {
	...
	// 图片按张计费与上游 token 成本独立结算。
	userBilled := UsageLogUserBilledCost(input)
