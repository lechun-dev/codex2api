...
func (db *DB) ensurePromptLogRetentionConfig(ctx context.Context) error {
	...
	if db.isMySQL() {
		ddl = strings.ReplaceAll(ddl, " CHECK (singleton_id = 1)", "")
		ddl = strings.ReplaceAll(ddl, "last_error TEXT NOT NULL DEFAULT ''", "last_error TEXT NULL")
		ddl = strings.ReplaceAll(ddl, "last_run_at TIMESTAMP NULL", "last_run_at DATETIME NULL")
...
func (db *DB) purgePromptLogs(ctx context.Context, cutoff time.Time, filter PromptLogPurgeFilter, batchSize int, pause time.Duration, includeEvents bool) (PromptLogPurgeResult, error) {
	...
	// 2026-09-09 coder(lq): Materialize limited candidates to avoid MySQL 5.6 LIMIT-IN and target-table deletion restrictions.
	logStmt := fmt.Sprintf(`DELETE FROM prompt_filter_logs WHERE id IN (
		SELECT id FROM (SELECT l.id FROM prompt_filter_logs l WHERE %s LIMIT $%d) AS purge_candidates)`, logWhere, len(logArgs)+1)
	...
	eventStmt := fmt.Sprintf(`DELETE FROM prompt_risk_events WHERE id IN (
		SELECT id FROM (SELECT e.id FROM prompt_risk_events e WHERE %s LIMIT $%d) AS purge_candidates)`, eventWhere, len(eventArgs)+1)
...
func (db *DB) purgeOrphanPromptRiskSources(ctx context.Context, cutoffArg interface{}, batchSize int, pause time.Duration, result *PromptLogPurgeResult) (int64, error) {
	stmt := `DELETE FROM prompt_risk_event_sources WHERE (source_type, source_id) IN (
		SELECT source_type, source_id FROM (SELECT s.source_type, s.source_id FROM prompt_risk_event_sources s
