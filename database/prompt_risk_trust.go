...
func (db *DB) ensurePromptRiskTrustTables(ctx context.Context) error {
	...
	for _, stmt := range []string{policyDDL, eventDDL} {
	...
	indexes := []struct {
		...
		{"prompt_risk_trust_policies", "idx_prompt_risk_trust_status_until", `CREATE INDEX idx_prompt_risk_trust_status_until ON prompt_risk_trust_policies(status, valid_until)`},
		{"prompt_risk_trust_events", "idx_prompt_risk_trust_events_policy", `CREATE INDEX idx_prompt_risk_trust_events_policy ON prompt_risk_trust_events(policy_id, created_at)`},
		{"prompt_risk_trust_events", "idx_prompt_risk_trust_events_subject", `CREATE INDEX idx_prompt_risk_trust_events_subject ON prompt_risk_trust_events(subject_type, subject_key, created_at)`},
	}
	...
	if db.isSQLite() {
		...
		for _, column := range []mysqlColumnDefinition{
			{table: "prompt_risk_trust_policies", name: "source", def: "VARCHAR(24) NOT NULL DEFAULT 'manual'"},
			{table: "prompt_risk_trust_policies", name: "model_review_count", def: "BIGINT NOT NULL DEFAULT 0"},
			{table: "prompt_risk_trust_policies", name: "last_model_review_at", def: "DATETIME NULL"},
		} {
...
func (db *DB) promptRiskReviewIdentitySQL() string {
...
func (db *DB) GetPromptRiskAdaptiveReviewBasis(ctx context.Context, subjectType, subjectKey string, since time.Time) (PromptRiskAdaptiveReviewBasis, error) {
	...
	WHERE subject_type=$1 AND subject_key=$2 AND is_person=TRUE AND created_at >= $3`, db.promptRiskReviewIdentitySQL()),
...
func (db *DB) promptRiskTrustEvidenceSummaries(ctx context.Context, since time.Time) ([]promptRiskTrustEvidenceSummary, error) {
	...
	GROUP BY subject_type, subject_key`, db.promptRiskReviewIdentitySQL()), PromptRiskSubjectNewAPIUser, since.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) promptRiskTrustEvidenceSummarySince(ctx context.Context, subjectKey string, since time.Time) (promptRiskTrustEvidenceSummary, error) {
	...
	FROM prompt_risk_events WHERE subject_type=$1 AND subject_key=$2 AND is_person=TRUE AND created_at >= $3`,
		db.promptRiskReviewIdentitySQL()), PromptRiskSubjectNewAPIUser, subjectKey, since.UTC()).Scan(&item.CleanCount, &item.PositiveCount, &firstClean, &lastClean)
	if err != nil {
		return item, err
	}
	if firstClean != nil {
