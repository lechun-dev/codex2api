...
const mysql56PromptRuleCandidatesDDL = `CREATE TABLE IF NOT EXISTS prompt_rule_candidates (
...
const mysql56PromptRuleCandidateEvidenceDDL = `CREATE TABLE IF NOT EXISTS prompt_rule_candidate_evidence (
	id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
	candidate_id BIGINT NOT NULL,
...
func (db *DB) ensurePromptRuleCandidatesTable(ctx context.Context) error {
	...
	if db.isSQLite() {
		...
		candidateDDL = mysql56PromptRuleCandidatesDDL
		evidenceDDL = mysql56PromptRuleCandidateEvidenceDDL
		for _, statement := range []string{candidateDDL, evidenceDDL} {
		...
		for _, index := range []struct {
			...
			{"prompt_rule_candidates", "idx_prompt_rule_candidates_status_updated", "CREATE INDEX idx_prompt_rule_candidates_status_updated ON prompt_rule_candidates(status, updated_at)"},
			{"prompt_rule_candidates", "idx_prompt_rule_candidates_source_last_seen", "CREATE INDEX idx_prompt_rule_candidates_source_last_seen ON prompt_rule_candidates(last_source, last_seen_at)"},
			{"prompt_rule_candidate_evidence", "idx_prompt_rule_candidate_evidence_candidate", "CREATE INDEX idx_prompt_rule_candidate_evidence_candidate ON prompt_rule_candidate_evidence(candidate_id, observed_at)"},
		} {
