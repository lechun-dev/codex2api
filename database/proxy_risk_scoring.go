...
func scanProxyRiskScoreSnapshot(scanner proxyRiskScanner) (ProxyRiskScoreSnapshot, error) {
	...
	var blacklistRaw, errorRaw, featuresRaw, responseRaw sql.NullString
	var checkedRaw, expiresRaw any
	if err := scanner.Scan(&snapshot.ID, &snapshot.ProxyID, &snapshot.ProfileID, &snapshot.Provider, &snapshot.ResolvedIP, &score, &snapshot.RiskLevel, &snapshot.Recommendation, &snapshot.ProxyType, &snapshot.IsVPN, &snapshot.IsTOR, &snapshot.IsDatacenter, &snapshot.IsBlacklisted, &blacklistRaw, &snapshot.ISP, &snapshot.Country, &snapshot.LatencyMS, &snapshot.Status, &errorRaw, &featuresRaw, &responseRaw, &checkedRaw, &expiresRaw); err != nil {
		return snapshot, err
	}
	if score.Valid {
	...
	snapshot.Error = errorRaw.String
	snapshot.FeaturesJSON = featuresRaw.String
	snapshot.RawResponseJSON = responseRaw.String
	_ = json.Unmarshal([]byte(blacklistRaw.String), &snapshot.BlacklistSource)
