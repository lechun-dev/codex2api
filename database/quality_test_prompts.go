...
func (db *DB) ensureQualityTestPromptSchema(ctx context.Context) error {
	if db.isMySQL() {
		// 2026-09-15 coder(lq): Keep indexed text within the MySQL 5.6 utf8mb4 key limit
		// and avoid unsupported defaults on TEXT/LONGTEXT columns.
		if _, err := db.conn.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS quality_test_prompts (
			...
			KEY idx_quality_test_prompts_updated (updated_at, id)
		) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`); err != nil {
			return err
		}
		return db.ensureMySQLIndex(ctx, "quality_test_prompts", "idx_quality_test_prompts_updated", `CREATE INDEX idx_quality_test_prompts_updated ON quality_test_prompts(updated_at, id)`)
	}
