...
func (db *DB) ensureQualityTestSchema(ctx context.Context) error {
	if db.isMySQL() {
		return db.ensureMySQLQualityTestSchema(ctx)
	}
...
func (db *DB) ensureMySQLQualityTestSchema(ctx context.Context) error {
	// 2026-09-15 coder(lq): MySQL 5.6 has no partial indexes, so a nullable unique
	// active_account_id represents the account only while its job occupies a slot.
	if _, err := db.conn.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS quality_test_jobs (
		...
		preset_kind VARCHAR(32) NOT NULL,
		preset_ref VARCHAR(191) NOT NULL,
		...
		UNIQUE KEY idx_quality_test_slot (slot),
		UNIQUE KEY idx_quality_test_active_account (active_account_id),
		KEY idx_quality_test_created (id)
	...
	for _, column := range []struct {
		...
		{name: "active_account_id", def: "BIGINT NULL"},
		{name: "preset_kind", def: "VARCHAR(32) NOT NULL DEFAULT ''"},
		{name: "preset_ref", def: "VARCHAR(191) NOT NULL DEFAULT ''"},
		{name: "preset_name", def: "VARCHAR(255) NOT NULL DEFAULT ''"},
	} {
		if err := db.ensureMySQLColumn(ctx, "quality_test_jobs", column.name, column.def); err != nil {
			return err
		}
	}
	if _, err := db.conn.ExecContext(ctx, `UPDATE quality_test_jobs SET active_account_id=account_id WHERE slot IS NOT NULL AND active_account_id IS NULL`); err != nil {
		return err
	}
	for _, index := range []struct {
		...
		{name: "idx_quality_test_slot", stmt: `CREATE UNIQUE INDEX idx_quality_test_slot ON quality_test_jobs(slot)`},
		{name: "idx_quality_test_active_account", stmt: `CREATE UNIQUE INDEX idx_quality_test_active_account ON quality_test_jobs(active_account_id)`},
		{name: "idx_quality_test_created", stmt: `CREATE INDEX idx_quality_test_created ON quality_test_jobs(id)`},
	} {
		if err := db.ensureMySQLIndex(ctx, "quality_test_jobs", index.name, index.stmt); err != nil {
			return err
		}
	}
	return db.ensureQualityTestPromptSchema(ctx)
...
func (db *DB) CreateQualityTestJob(ctx context.Context, job QualityTestJob) (*QualityTestJob, error) {
	...
	if db.isMySQL() {
		query = `INSERT IGNORE INTO quality_test_jobs(slot,active_account_id,account_id,account_name,plan_type,channel,model,reasoning_effort,prompt,status,output,metrics_json,error,created_at,updated_at,deadline_at,preset_kind,preset_ref,preset_name)
		 VALUES($1,$2,$2,$3,$4,$5,$6,$7,$8,'running','','{}','',$9,$9,$10,$11,$12,$13)`
	}
	now := time.Now().UTC()
...
func (db *DB) ExpireQualityTests(ctx context.Context, now time.Time) error {
	setActiveAccount := ""
	if db.isMySQL() {
		setActiveAccount = ",active_account_id=NULL"
	}
	_, err := db.conn.ExecContext(ctx, `UPDATE quality_test_jobs SET status='interrupted',slot=NULL`+setActiveAccount+`,error='检测进程中断或超时，请重新发起检测',completed_at=$1,updated_at=$1 WHERE slot IS NOT NULL AND deadline_at<$2`, db.timeArg(now), db.timeArg(now.Add(-30*time.Second)))
	return err
}
...
func (db *DB) FinishQualityTest(ctx context.Context, job QualityTestJob) error {
	...
	_, err = db.conn.ExecContext(ctx, `UPDATE quality_test_jobs SET status=CASE WHEN status='cancelling' THEN 'stopped' ELSE $1 END,slot=NULL`+setActiveAccount+`,output=$2,metrics_json=$3,error=CASE WHEN status='cancelling' THEN '' ELSE $4 END,updated_at=$5,completed_at=$5 WHERE id=$6 AND slot IS NOT NULL`, job.Status, job.Output, string(metrics), job.Error, db.timeArg(time.Now()), job.ID)
	return err
}
