...
func TestSQLiteUsageLogsHasAPIKeyColumns(t *testing.T) {
	...
	for _, name := range []string{"api_key_id", "api_key_name", "api_key_masked", "client_ip", "session_id", "conversation_id", "previous_response_id", "request_text", "image_count", "image_width", "image_height", "image_bytes", "image_format", "image_size", "effective_model", "compact", "account_billed", "user_billed", "is_retry_attempt", "attempt_index", "upstream_error_kind", "error_message"} {
...
func TestSQLiteUsageLogStoresRequestContentFields(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "codex2api.db")
	...
	var sessionID, conversationID, previousResponseID, requestText string
	if err := db.conn.QueryRowContext(ctx, `SELECT session_id, conversation_id, previous_response_id, request_text FROM usage_logs LIMIT 1`).Scan(
		&sessionID, &conversationID, &previousResponseID, &requestText,
	); err != nil {
		t.Fatalf("查询 usage_logs 会话字段返回错误: %v", err)
	}
	if sessionID != "session-123" || conversationID != "conversation-456" || previousResponseID != "resp_789" || requestText != "hello\nworld" {
		t.Fatalf("usage_logs 会话字段 = (%q, %q, %q, %q)", sessionID, conversationID, previousResponseID, requestText)
	}
}
