...
func (db *DB) SaveVisibleChannelsConfig(ctx context.Context, cfg VisibleChannelsConfig) error {
	...
	return db.withSQLiteWriteLock(ctx, func() error {
		...
		`
		if db.isMySQL() {
			// 2026-09-06 coder(lq): MySQL 5.6 does not support ON CONFLICT; INSERT IGNORE preserves the insert-if-missing behavior.
			insertQuery = `INSERT IGNORE INTO system_settings (id) VALUES (1)`
		}
		if _, err := db.conn.ExecContext(ctx, insertQuery); err != nil {
			return err
		}
