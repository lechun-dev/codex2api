...
import type { APIKeyRow, APIKeyTokenStat, OpsErrorSummary, SystemSettings, UsageAPIKeyStat, UsageEndpointStat, UsageFeatureStats, UsageLog, UsageModelStat, UsageStats, UsageDailyTokenStats, PromptFilterLog, PromptPolicyIncidentDetailResponse } from '../types'
...
import { Activity, Box, Clock, Zap, Sparkles, AlertTriangle, Search, Brain, DatabaseZap, DatabaseBackup, X, Image as ImageIcon, Info, CircleDollarSign, BarChart3, KeyRound, Route, SlidersHorizontal, ShieldAlert, RefreshCw, ChevronDown, RotateCcw, MoreHorizontal, PlugZap, FlaskConical } from 'lucide-react'
  ...
  return (
    <DistributionPanel
      ...
      action={stats.length > 0 && onMore ? (
        <Button variant="ghost" size="sm" onClick={onMore} className="h-8 gap-1 px-2 text-xs">
  ...
  return (
    <PanelShell>
      <PanelHeader
        ...
        trailing={action ?? <span className="usage-panel-badge">{t('usage.requestShare')}</span>}
      />
...
export default function Usage() {
  ...
  const dailyStatsAbortRef = useRef<AbortController | null>(null)
  const dailyStatsRequestSeqRef = useRef(0)
  ...
  const loadDailyTokenUsage = useCallback(async (silent = false) => {
    ...
    dailyStatsAbortRef.current = controller
    const requestSeq = ++dailyStatsRequestSeqRef.current
    setDailyStatsLoading(!silent)
    try {
      const { start, end } = resolveRangeISO(timeRange, customRange)
      ...
      if (controller.signal.aborted || requestSeq !== dailyStatsRequestSeqRef.current) return
      ...
      if (dailyStatsAbortRef.current === controller) {
        dailyStatsAbortRef.current = null
  ...
  }, [loadDailyTokenUsage, reloadSilently])
