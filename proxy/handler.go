...
type Handler struct {
	...
	// 2026-09-03 coder(lq): 会话记录器独立于请求路径，避免串到热路径上的 DB 写。
...
func forwardGrokNativeResponseObserved(readCtx context.Context, c *gin.Context, resp *http.Response, protocol GrokProtocol, streaming bool, startedAt time.Time, firstVisible func(), output io.Writer, outputFlusher http.Flusher, observe func([]byte)) (*UsageInfo, streamOutcome, bool, int) {
	...
	readErr := readRawGrokSSEFramesWithContinuousRetryKeepalive(readCtx, resp.Body, func(frame rawGrokSSEFrame) bool {
		...
		becameVisible := isVisible && !visible
		if becameVisible {
			visible = true
			if firstVisible != nil {
				firstVisible()
		...
		outputFlusher.Flush()
		if becameVisible && !keepaliveStarted {
			keepaliveStarted = true
			keepaliveFrame := downstreamSSEKeepaliveFrameForRequest(c)
			// 2026-09-15 coder(lq): 原生 Grok 流首帧后启动下游保活，并复用写锁防止心跳与上游 SSE 帧交叉。
			stopDownstreamKeepalive = startDownstreamSSEKeepalive(readCtx, downstreamSSEKeepaliveInterval, func() bool {
				downstreamMu.Lock()
				defer downstreamMu.Unlock()
				if writeErr != nil {
					return false
...
func NewHandler(store *auth.Store, db *database.DB, cfg *config.Config, deviceCfg *DeviceProfileConfig) *Handler {
	...
	handler := &Handler{
		...
		cfg:          cfg,
		deviceCfg:    deviceCfg,
		convRecorder: newConversationRecorder(db),
